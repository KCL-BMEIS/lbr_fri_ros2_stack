...
BooleanCommandInterface::BooleanCommandInterface(
...
void BooleanCommandInterface::buffered_command_to_fri(fri_command_t_ref command,
  ...
  // 将每个布尔 IO 的值写入到命令中
  for (size_t i = 0; i < command_target_.boolean_io_names.size(); ++i) {
    const char *io_name = command_target_.boolean_io_names[i].c_str();  // IO 名称
    bool io_value       = command_target_.boolean_io_values[i];         // IO 值

    // 调用 FRI 接口设置 IO 值
    command.setBooleanIOValue(io_name, io_value);
  }
}
